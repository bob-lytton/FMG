...
def get_id_to_num(json_datas, filtered_list, filtered_name, id_name, multi_value):
    """
    read from json_datas and return the position it first appears as its index
    id for data_id, num for index
    """
    num_to_id = {}
    id_to_num = {}
    tot = 0
    for data in json_datas:
        ...
        for data_id in data_ids:
            if data_id not in id_to_num:
                num_to_id[tot] = data_id    # input num, output id
                id_to_num[data_id] = tot    # input id, output num
                tot = tot + 1
